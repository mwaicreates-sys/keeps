...
export function AddStoryTile({ name, avatarUrl }: { name: string; avatarUrl?: string | null }) {
  return (
    <div className="flex w-[88px] shrink-0 flex-col items-center gap-1.5">
      <div className="relative h-[118px] w-[88px] overflow-hidden rounded-[20px] bg-[#f2efe9]">
        <div className="grid h-full w-full place-items-center">
          <Avatar name={name} url={avatarUrl} size={44} />
        </div>
        <span className="absolute bottom-1.5 right-1.5 grid h-7 w-7 place-items-center rounded-full bg-[#3b82f6] text-white ring-2 ring-white">
          <Plus size={15} strokeWidth={2.5} />
        </span>
      </div>
      <span className="text-[12px] font-medium text-[#7c766c]">Add story</span>
    </div>
  );
}
...
export function StoryTile({ story, unseen, onOpen }: { story: StoryWithAuthor; unseen: boolean; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="flex w-[88px] shrink-0 flex-col items-center gap-1.5 text-left">
      <div
        className="relative h-[118px] w-[88px] overflow-hidden rounded-[20px] bg-[#f2efe9] ring-2"
      ...
      <span className="max-w-[88px] truncate text-[12px] font-medium text-[#3a362f]">{story.author.display_name}</span>
    </button>
  );
}
...
export function StoryPlaceholderTile({ member }: { member: Tables<"profiles"> }) {
  return (
    <div className="flex w-[88px] shrink-0 flex-col items-center gap-1.5">
      <div className="grid h-[118px] w-[88px] place-items-center rounded-[20px] bg-[#f2efe9]">
        <Avatar name={member.display_name} url={member.avatar_url} size={44} />
      </div>
      <span className="max-w-[88px] truncate text-[12px] font-medium text-[#a39d92]">{member.display_name}</span>
    </div>
  );
}
