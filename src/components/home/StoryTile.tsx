...
export function AddStoryTile({ name, avatarUrl }: { name: string; avatarUrl?: string | null }) {
  return (
    <div className="flex w-[104px] shrink-0 flex-col items-center gap-2">
      <div className="relative h-[140px] w-[104px] overflow-hidden rounded-[26px] bg-[#f2efe9]">
        <div className="grid h-full w-full place-items-center">
          <Avatar name={name} url={avatarUrl} size={52} />
        </div>
        <span className="absolute bottom-2 right-2 grid h-9 w-9 place-items-center rounded-full bg-[#3b82f6] text-white ring-2 ring-white">
          <Plus size={18} strokeWidth={2.5} />
        </span>
      </div>
      <span className="text-[14px] font-medium text-[#7c766c]">Add story</span>
    </div>
  );
}
...
export function StoryTile({ story, unseen, onOpen }: { story: StoryWithAuthor; unseen: boolean; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="flex w-[104px] shrink-0 flex-col items-center gap-2 text-left">
      <div
        className="relative h-[140px] w-[104px] overflow-hidden rounded-[26px] bg-[#f2efe9] ring-[3px]"
        ...
        {story.media_url ? (
          ...
          <div className="grid h-full w-full place-items-center bg-gradient-to-br from-[#fde2e4] to-[#e2ece9]">
            {story.type === "song" ? (
              ...
              <TypeIcon size={22} className="text-white/80" />
            )}
          </div>
        )}
        <span className="absolute left-2 top-2 rounded-full ring-[3px] ring-white">
          <Avatar name={story.author.display_name} url={story.author.avatar_url} size={36} />
        </span>
      </div>
      <span className="max-w-[104px] truncate text-[14px] font-medium text-[#3a362f]">{story.author.display_name}</span>
    </button>
  );
}
...
export function StoryPlaceholderTile({ member }: { member: Tables<"profiles"> }) {
  return (
    <div className="flex w-[104px] shrink-0 flex-col items-center gap-2">
      <div className="grid h-[140px] w-[104px] place-items-center rounded-[26px] bg-[#f2efe9]">
        <Avatar name={member.display_name} url={member.avatar_url} size={52} />
      </div>
      <span className="max-w-[104px] truncate text-[14px] font-medium text-[#a39d92]">{member.display_name}</span>
    </div>
  );
}
