...
export function AddStoryTile({ name, avatarUrl }: { name: string; avatarUrl?: string | null }) {
  return (
    <div className="flex w-24 shrink-0 flex-col items-center gap-1.5">
      <div className="relative h-32 w-24 overflow-hidden rounded-[24px] bg-[#f2efe9]">
        <div className="grid h-full w-full place-items-center">
          <Avatar name={name} url={avatarUrl} size={44} />
        </div>
        <span className="absolute bottom-2 right-2 grid h-8 w-8 place-items-center rounded-full bg-[#3b82f6] text-white ring-2 ring-white">
          <Plus size={16} strokeWidth={2.5} />
        </span>
      </div>
      <span className="text-[13px] text-[#a39d92]">Add story</span>
    </div>
  );
}
...
export function StoryTile({ story, unseen, onOpen }: { story: StoryWithAuthor; unseen: boolean; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="flex w-24 shrink-0 flex-col items-center gap-1.5 text-left">
      <div
        className="relative h-32 w-24 overflow-hidden rounded-[24px] bg-[#f2efe9] ring-2"
      ...
      >
        ...
        <span className="absolute left-2 top-2 rounded-full ring-2 ring-white">
          <Avatar name={story.author.display_name} url={story.author.avatar_url} size={32} />
        </span>
      </div>
      <span className="max-w-[96px] truncate text-[13px] text-[#7c766c]">{story.author.display_name}</span>
    </button>
  );
}
...
export function StoryPlaceholderTile({ member }: { member: Tables<"profiles"> }) {
  return (
    <div className="flex w-24 shrink-0 flex-col items-center gap-1.5">
      <div className="grid h-32 w-24 place-items-center rounded-[24px] bg-[#f2efe9]">
        <Avatar name={member.display_name} url={member.avatar_url} size={44} />
      </div>
      <span className="max-w-[96px] truncate text-[13px] text-[#a39d92]">{member.display_name}</span>
    </div>
  );
}
